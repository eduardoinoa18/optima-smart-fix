...
export default function WhatsAppButton() {
  ...
  return (
    <a
      ...
      className={`fixed z-50 bottom-6 right-6 w-14 h-14 rounded-full shadow-xl flex items-center justify-center transition-transform duration-200 ${
        isConfigured ? 'hover:scale-105' : 'opacity-60 cursor-not-allowed'
      } bg-[#25D366]`}
      {...(!isConfigured && { onClick: (e) => e.preventDefault() })}
    >
      <svg
